...
var_decl: 
      T_VAR T_ID ':' type ';'				{
      										...
      										$$=temp;env_insert(prog_env,tempSym,$4);							
      										}
...
param_decl: 
      T_ID ':' type							{TypedId temp;
      										...
      										temp.type = $3;
      										$$= &temp;env_insert(prog_env,temp.id,temp.type);
      										}
...
field_decl: 
      T_ID ':' type							{Type * temp = $3; $$=type_new(*temp);
									Symbol tempSym = symbol_field($1);
									env_insert(prog_env,tempSym,$3}
...
fun_decl: 
      T_FUNCTION T_ID '(' param_decls ')' ':' type '{' var_decls stmts '}'		{Type tempType = type_fun($4, $7);
      											...
      											$$=temp;
											Env* fun_env = env_new();
											env_insert_fun(prog_env,symbol_fun($2),&tempType,fun_env,$10);
											}
      | T_FUNCTION T_ID '(' param_decls ')' '{' var_decls stmts '}'			{Type tempType = type_fun($4, NULL);
      											struct decl * temp = decl_new(symbol_fun($2),&tempType, NULL, $7, $8);      												$$=temp;
											Env* fun_env = env_new();
											env_insert_fun(prog_env,symbol_fun($2),&tempType,fun_env,$8);}
...
array_lit:									
      ...
      | T_STR								{$$= exp_str_new($1);
									/*Symbol T_STR_symbol = symbol_var($1);
									Type* T_STR_type = type_new($1);
									env_insert(prog_env,T_STR_symbol,T_STR_type);*/}
