...
// depending on the "kind" of the node.

/*** Declarations. ***/

struct decl {
      // The "kind" of decl (var, typename, function) is carried around in the
      // symbol namespace, so we don't need a separate "kind" field.
      Symbol id;
...
// none) and the last two are only for function defs (local decls and the
// function body).
struct decl* decl_field_new(const char* id, Type*);
struct decl* decl_param_new(const char* id, Type*);
struct decl* decl_var_new(const char* id, Type*, struct exp*);
struct decl* decl_typename_new(const char* id, Type*);
struct decl* decl_fun_new(const char* id, Type*, GList* decls, GList* stmts);
...
struct decl* decl_copy_deep(const struct decl*);
GList* decls_copy_deep(GList*);
struct exp* exp_copy_deep(const struct exp*);
GList* exps_copy_deep(GList*);
struct field_init* field_init_copy_deep(const struct field_init*);
GList* field_inits_copy_deep(GList*);
struct stmt* stmt_copy_deep(const struct stmt*);
